...
@click.option('--precond',       help='Preconditioning & loss function', metavar='vp|ve|edm|blur',  type=click.Choice(['vp', 've', 'edm', 'blur']), default='edm', show_default=True)
...
# Extension
@click.option('--eff-attn',      help='Enable efficient attention module',                          type=bool, default=False, show_default=True)

# Aim logger related
@click.option('--aim-repo',      help='the repo of aim-logger, default to ./aim/', metavar='STR',   type=str, default="")
@click.option('--exp-name',      help='experiment\'s name in aim-logger', metavar='STR',            type=str, default="")
@click.option('--exp-tag',       help='experiment\'s tag in aim-logger', metavar='STR',             type=str, default="")
@click.option('--no-aim',        help='Don\'t use aim logger',                                      is_flag=True)
...
@click.option('--blur-sigma-max',help='Maximum sigma of blurring schedule', metavar='FLOAT',        type=click.FloatRange(min=0), default=3, show_default=True)
...
def main(**kwargs):
    ...
    c.aim_logger_kwargs = dnnlib.EasyDict(experiment=opts.exp_name if opts.exp_name != "" else os.path.basename(opts.outdir), 
                                          repo=opts.aim_repo if opts.aim_repo != "" else None, tag=opts.exp_tag, 
                                          forbid=opts.no_aim)
    ...
    if opts.precond == 'vp':
        ...
        c.loss_kwargs.block_scale = opts.block_scale
        c.loss_kwargs.up_scale = opts.up_scale
        c.loss_kwargs.prob_length = opts.prob_length
        c.loss_kwargs.blur_sigma_max = opts.blur_sigma_max
    else:

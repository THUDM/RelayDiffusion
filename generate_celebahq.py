...
def blur_sampler(
    ...
    num_steps=18, sigma_min=0.008, sigma_max=80, rho=7,
    truncation_sigma=0.95, truncation_t=1.05, up_scale=4, cfg_scale=1,
    ...
    for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])): # 0, ..., N-1
        ...
        alpha_cur = get_alpha_t(t_hat, up_scale, latents.device, prob_length=truncation_t, blur_sigma_max=blur_sigma_max)
        
        u_cur = dct_2d(x_cur, up_scale, norm='ortho')
        u_0 = dct_2d(denoised, up_scale, norm='ortho')
        d_cur = (u_cur - u_0) / t_hat
        
        gamma = (1 - s_noise**2)**0.5 * t_next / t_hat
        
        if s_block > 0:
            x_next = idct_2d((alpha_next + gamma - gamma * alpha_cur) * u_cur \
                + t_hat * (gamma * alpha_cur - alpha_next) * d_cur, up_scale, norm='ortho') \
        ...
        if i < num_steps - 1:
            ...
            if s_block > 0:
                x_next = idct_2d((alpha_next + gamma - gamma * alpha_cur) * u_cur \
                    + t_hat * (gamma * alpha_cur - alpha_next) * (d_cur + d_prime) / 2, up_scale, norm='ortho') \
                    + s_noise * t_next * (randn_like(x_next) + s_block * block_noise(latents, randn_like, up_scale, device=latents.device))
            else:
                x_next = idct_2d((alpha_next + gamma - gamma * alpha_cur) * u_cur \
                    + t_hat * (gamma * alpha_cur - alpha_next) * (d_cur + d_prime) / 2, up_scale, norm='ortho') \
...
def edm_sampler(
    ...
    S_churn=0, S_min=0, S_max=float('inf'), S_noise=1, sde_scale=0.05, cfg_scale=1
):
...
@click.option('--seeds',                     help='Random seeds (e.g. 1,2,5-10)', metavar='LIST',                     type=parse_int_list, default='80000-109999', show_default=True)
...
@click.option('--truncation_t_second',       help='Truncation point of time schedule', metavar='FLOAT',               type=click.FloatRange(min=0, min_open=True), default=1.05, show_default=True)
